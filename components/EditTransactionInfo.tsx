...
export default function EditTransactionInfo({ transaction, pickedWallet, pickedTag, type} : Props)  {
  ...
  const [wallet, setWallet] = useState<Wallet>(pickedWallet ?? undefined)
  const [walletPickVisible, setWalletPickVisible] = useState(false)
  const [tag,setTag] = useState<Tag>(pickedTag ?? undefined)
  const [tagPickVisible, setTagPickVisible] = useState(false)
  const [date,setDate] = useState<Date>(new Date())
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [formattedDate, setFormattedDate] = useState(transaction?.date ?? '');
  ...
  async function handlePress() {
    try {
      ...
      await db.updateTransaction(transaction!.id,newValue,desc,formattedDate,tag!.id,wallet!.id)
